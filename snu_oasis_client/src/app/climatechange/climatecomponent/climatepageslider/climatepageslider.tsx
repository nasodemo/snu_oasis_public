...
import styles from './climatepageslider.module.css';
import 'slick-carousel/slick/slick-theme.css';
import 'slick-carousel/slick/slick.css';
import Link from 'next/link';
import ShowInputform from '@/app/[word]/components/(pageslider)/(pageslidercomponent)/(Inputform)/ShowInputform';
import Inputandsaveshow from '../../../[word]/components/(Inputandsave)/Inputandsaveshow';
import data from '../../../../../public/data.json'
import Quesitons from '../../../[word]/components/{questions)/questions';
import { usePathname } from 'next/navigation';

const ClimatePageslider: React.FC = () => {
        ...
        <div className={styles.pagesliderWrapper}>
            ...
            <Slider {...settings} ref={sliderRef}>
                <div>
                  ...
                  <h2>&nbsp;8 관점</h2>
                  <p>&nbsp;제시된 8 관점을 통해 기후변화에 대해 탐구해 보아요. </p>
                   
                  <div className={styles.flexContainer}>
                    <Link href={`climatechange/liberalarts`} className={styles.textdeconone}>
                      <form className={`${styles.clickbox} ${styles.liberalarts}`}>인문학</form>
                    </Link>
                    <Link href={`climatechange/education`} className={styles.textdeconone}>
                      <form className={`${styles.clickbox} ${styles.education}`}>교육학</form>
                    </Link>
                    <Link href={`climatechange/socience`} className={styles.textdeconone}>
                      <form className={`${styles.clickbox} ${styles.socience}`}>사회과학</form>
                    </Link>

                  </div>
                  <div className={styles.flexContainer}>
                    <Link href={`climatechange/naturescience`} className={styles.textdeconone}>
                      <form className={`${styles.clickbox} ${styles.naturescience}`}>자연과학</form>
                    </Link>
                    <Link href={`climatechange/`} className={styles.textdeconone}>
    ...
    export default ClimatePageslider;
